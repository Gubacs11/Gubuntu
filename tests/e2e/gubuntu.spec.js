...
test("Chaos Works produces, sells, keeps, reloads, and resets through the UI",async({page,context})=>{
  ...
  const id=await page.evaluate(()=>JSON.parse(localStorage.getItem("gubuntu-arcade-v1")).profiles[0].id),card=page.locator('[data-game-card="chaosworks"]');await card.scrollIntoViewIfNeeded();await card.press("Enter");await expect(page.locator("#cw-produce")).toBeVisible();await expect(page.locator(".cw-recipes .cw-part-art svg")).toHaveCount(4);await expect(page.locator(".cw-blueprint .cw-part-bolt")).toBeVisible();
  const readState=target=>target.evaluate(()=>JSON.parse(localStorage.getItem("gubuntu-arcade-v1")).profiles[0].chaosWorks);
  await page.locator("#cw-produce").click();await expect(page.locator('#cw-inspection [data-cw-action="sell"]')).toBeVisible({timeout:10000});await expect(page.locator("#cw-inspection .cw-part-bolt.cw-art-hero")).toBeVisible();await expect(page.locator(".cw-odds")).toContainText("1 IN");let state=await readState(page);expect(state.economy.totalProductionCost).toBe(18);const firstValue=state.recentProducts[0].value;await page.locator('[data-cw-action="sell"]').click();state=await readState(page);expect(state.totalSold).toBe(1);expect(state.economy.totalRevenue).toBe(firstValue);expect(state.recentProducts[0].action).toBe("SOLD");
  await page.locator("#cw-produce").click();await expect(page.locator('#cw-inspection [data-cw-action="scrap"]')).toBeVisible({timeout:10000});const cashBeforeScrap=(await readState(page)).cash;await page.locator('[data-cw-action="scrap"]').click();state=await readState(page);expect(state.cash-cashBeforeScrap).toBe(6);expect(state.cash-cashBeforeScrap).toBeLessThan(18);expect(state.economy.totalScrapValue).toBe(6);expect(state.recentProducts[0].action).toBe("SCRAPPED");
  for(let i=0;i<2;i++){await page.locator("#cw-produce").click();await expect(page.locator('#cw-inspection [data-cw-action="keep"]')).toBeVisible({timeout:10000});await page.locator('[data-cw-action="keep"]').click()}await expect(page.locator(".cw-warehouse .cw-product-card")).toHaveCount(2);await page.locator("[data-cw-lock]").first().click();state=await readState(page);expect(state.keptProducts.some(item=>item.locked)).toBe(true);await page.locator('#game-dialog [data-close="game-dialog"]').click();await expect(page.locator("#game-dialog")).not.toHaveAttribute("open","");
  await page.close({runBeforeUnload:false});const restored=await context.newPage();collectRuntimeErrors(restored,errors);await openSavedProfile(restored,id);const restoredCard=restored.locator('[data-game-card="chaosworks"]');await restoredCard.scrollIntoViewIfNeeded();await restoredCard.press("Enter");await expect(restored.locator(".cw-warehouse .cw-product-card")).toHaveCount(2);await expect(restored.locator("[data-cw-lock].active")).toHaveCount(1);await expect(restored.locator("[data-cw-warehouse-sell]:disabled")).toHaveCount(1);await expect(restored.locator("[data-cw-warehouse-scrap]:disabled")).toHaveCount(1);await restored.locator("[data-cw-compare]").nth(0).click();await restored.locator("[data-cw-compare]").nth(1).click();await expect(restored.locator(".cw-compare-grid")).toBeVisible();
  await restored.locator('[data-cw-number="sellAbove"]').fill("0");await restored.locator('[data-cw-number="sellAbove"]').press("Tab");await restored.locator('[data-cw-setting="autoSell"]').check();const soldBefore=(await readState(restored)).totalSold;await restored.evaluate(()=>window.__CHAOS_WORKS_TEST_RARITY__="COMMON");await restored.locator("#cw-produce").click();await expect.poll(async()=>(await readState(restored)).totalSold,{timeout:10000}).toBe(soldBefore+1);await expect(restored.locator(".cw-history>button").first()).toContainText("SOLD");
  const baseline=(await readState(restored)).totalProduced;await restored.locator("#cw-produce").click();await restored.locator('#game-dialog [data-close="game-dialog"]').click();await expect(restored.locator("#game-dialog")).not.toHaveAttribute("open","");await restored.waitForTimeout(3400);expect((await readState(restored)).totalProduced).toBe(baseline);
  await restoredCard.scrollIntoViewIfNeeded();await restoredCard.press("Enter");await expect(restored.locator("#cw-produce")).toBeVisible();await restored.locator('#game-dialog [data-close="game-dialog"]').click();await expect(restored.locator("#game-dialog")).not.toHaveAttribute("open","");await restored.locator('[data-nav="profile"]').click();restored.once("dialog",dialog=>dialog.accept());await restored.locator("#reset-player").click();const reset=await restored.evaluate(()=>JSON.parse(localStorage.getItem("gubuntu-arcade-v1")).profiles[0].chaosWorks);expect(reset.cash).toBe(500);expect(reset.totalProduced).toBe(0);expect(reset.keptProducts).toEqual([]);expect(reset.economy.totalProductionCost).toBe(0);expect(reset.economy.totalRevenue).toBe(0);expect(reset.upgrades.materialRecovery).toBe(0);expect(reset.productionSettings.protectDiscoveries).toBe(true);expect(errors,errors.join("\n")).toEqual([]);await restored.close({runBeforeUnload:false});
});

test("Chaos Works exceptional reveals complete safely and never trap Close Game",async({page})=>{
  const errors=collectRuntimeErrors(page);await preparePage(page,{name:"CHAOS REVEAL"});const card=page.locator('[data-game-card="chaosworks"]');await card.scrollIntoViewIfNeeded();await card.press("Enter");
  for(const rarity of ["LEGENDARY","PROTOTYPE"]){await page.evaluate(value=>window.__CHAOS_WORKS_TEST_RARITY__=value,rarity);await page.locator("#cw-produce").click();await expect(page.locator(".cw-reveal")).toBeVisible({timeout:10000});await expect(page.locator(".cw-reveal")).not.toBeVisible({timeout:3500});await expect(page.locator(`#cw-inspection .rarity-${rarity.toLowerCase()}`)).toBeVisible();await page.locator('[data-cw-action="sell"]').click()}
  await page.evaluate(()=>window.__CHAOS_WORKS_TEST_RARITY__="ANOMALOUS");await page.locator("#cw-produce").click();await expect(page.locator(".cw-reveal")).toBeVisible({timeout:10000});await page.locator('#game-dialog [data-close="game-dialog"]').click();await expect(page.locator("#game-dialog")).not.toHaveAttribute("open","");await page.waitForTimeout(2200);expect(errors,errors.join("\n")).toEqual([]);
});

test("Chaos Works factory floor is usable on all target mobile viewports",async({browser})=>{
  for(const viewport of [{width:360,height:800},{width:390,height:844},{width:412,height:915},{width:844,height:390}]){const context=await browser.newContext({viewport,isMobile:true,hasTouch:true}),page=await context.newPage(),errors=collectRuntimeErrors(page);await preparePage(page,{name:`CW${viewport.width}`,device:"mobile"});const card=page.locator('[data-game-card="chaosworks"]');await card.scrollIntoViewIfNeeded();await card.press("Enter");await expect(page.locator("#cw-produce")).toBeVisible();await expect(page.locator(".cw-comparison")).toBeVisible();await expect(page.locator(".cw-automation")).toBeVisible();await expect(page.locator(".cw-history")).toBeVisible();await page.evaluate(()=>window.__CHAOS_WORKS_TEST_RARITY__="COMMON");await page.locator("#cw-produce").click();await expect(page.locator(".cw-odds")).toBeVisible({timeout:10000});expect(await page.locator(".cw-shell").evaluate(node=>node.scrollWidth<=node.clientWidth+3),`Chaos Works overflow at ${viewport.width}x${viewport.height}`).toBe(true);await page.locator('#game-dialog [data-close="game-dialog"]').click();await expect(page.locator("#game-dialog")).not.toHaveAttribute("open","");expect(errors,errors.join("\n")).toEqual([]);await context.close()}
});
