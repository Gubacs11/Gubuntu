...
test("Canonical reset removes all known and unknown progression while preserving identity and entitlement",()=>{
  const {createDefaultPlayer,createDefaultChaosWorksState,resetPlayerState}=load("player-state.js").GubuntuPlayerState;
  ...
  assert.deepEqual([...player.vehicles],["compact"]);assert.equal(player.openRoadJobs.completed,0);assert.equal(player.starfarer.atlas.length,0);assert.equal(player.fishing.bucket.length,0);assert.equal(player.salvager.xp,0);assert.equal(player.voidMiner.credits,0);assert.equal(player.tdProgress.level,1);assert.equal(player.cardLounge.reputation,0);assert.deepEqual(player.chaosWorks,createDefaultChaosWorksState());assert.equal(player.chaosWorks.unknownChaosProgress,undefined);assert.equal(player.battlePass.xp,0);assert.equal(player.playTimeMs,0);
});
...
test("Chaos Works save normalization preserves progress and bounds collections",()=>{
  const api=load("player-state.js").GubuntuPlayerState,input={version:1,cash:900,totalProduced:42,selectedRecipe:"gear",keptProducts:Array.from({length:90},(_,i)=>({id:`p${i}`,locked:i===89})),recentProducts:Array.from({length:30},(_,i)=>({id:`r${i}`})),upgrades:{precisionTooling:3},productionSettings:{autoKeepRare:true,alertAnomalous:true}};const state=api.normalizeChaosWorksState(input);assert.equal(state.cash,900);assert.equal(state.totalProduced,42);assert.equal(state.selectedRecipe,"gear");assert.equal(state.upgrades.precisionTooling,3);assert.equal(state.upgrades.materialRecovery,0);assert.equal(state.keptProducts.length,70);assert.equal(state.keptProducts.at(-1).locked,true);assert.equal(state.recentProducts.length,16);assert.equal(state.productionSettings.autoKeep,true);assert.equal(state.productionSettings.alertMinimumRarity,"ANOMALOUS");assert.equal(state.version,2);
});

test("Chaos Works weighted traits are ordered, rare, and conflict safe",()=>{
  const context={GubuntuGames:{register:()=>{}}};load("games/chaos-works.js",context);const core=context.ChaosWorksCore;let seed=17;const rng=()=>{seed=(seed*1664525+1013904223)>>>0;return seed/4294967296},counts={};for(let i=0;i<60000;i++)for(const id of core.chooseTraits("gear",rng,{})){counts[id]=(counts[id]||0)+1}assert.ok(counts.precision>counts.experimental*8,JSON.stringify(counts));assert.ok(counts.experimental>counts.impossible*3,JSON.stringify(counts));assert.ok(counts.experimental>180&&counts.experimental<1100,`Experimental ${counts.experimental}`);assert.ok(counts.impossible>25&&counts.impossible<250,`Impossible ${counts.impossible}`);for(let i=0;i<12000;i++){const traits=core.chooseTraits("gear",rng,{experimentalEngineering:5});for(const id of traits)for(const conflict of core.TRAITS[id].conflictsWith||[])assert.equal(traits.includes(conflict),false,`${id} conflicts with ${conflict}`)}
});

test("Chaos Works can deterministically exercise every rarity and derives odds",()=>{
  const context={GubuntuGames:{register:()=>{}}};load("games/chaos-works.js",context);const core=context.ChaosWorksCore;for(const rarity of ["COMMON","UNCOMMON","RARE","EPIC","LEGENDARY","PROTOTYPE","ANOMALOUS"]){const product=core.generateProduct("bolt",()=>.5,{forceRarity:rarity,traits:[]});assert.equal(product.rarity,rarity);assert.ok(Number.isFinite(product.rollScore));assert.ok(Number.isInteger(product.estimatedOneIn)&&product.estimatedOneIn>=2);if(rarity==="ANOMALOUS")assert.ok(product.anomaly)}const ordinary=core.generateProduct("gear",()=>.5,{forceRarity:"COMMON",traits:[]}),impossible=core.generateProduct("gear",()=>.5,{forceRarity:"LEGENDARY",traits:["impossible"]});assert.ok(impossible.rollScore>ordinary.rollScore);assert.ok(impossible.estimatedOneIn>ordinary.estimatedOneIn);
});

test("Chaos Works economy makes scrap partial and Material Recovery meaningful",()=>{
  const context={GubuntuGames:{register:()=>{}}};load("games/chaos-works.js",context);const core=context.ChaosWorksCore,product=core.generateProduct("spring",()=>.5,{traits:[]}),cost=core.PRODUCT_TYPES.spring.cost,base=core.scrapRefund(cost,0),improved=core.scrapRefund(cost,4);assert.equal(base,Math.round(cost*.35));assert.ok(base<cost);assert.equal(improved,Math.round(cost*.65));assert.ok(improved>base&&improved<cost);assert.ok(core.scrapYield(product,4)>core.scrapYield(product,0));for(let i=0;i<100;i++){const item=core.generateProduct("bearing",Math.random);assert.ok(Number.isFinite(item.value)&&item.value>=1)}
});

test("Chaos Works automation has deterministic priority and discovery safety",()=>{
  const context={GubuntuGames:{register:()=>{}}};load("games/chaos-works.js",context);const core=context.ChaosWorksCore,base={autoKeep:true,keepMinimumRarity:"LEGENDARY",autoSell:true,sellAbove:10,autoScrap:true,scrapBelow:90,alertMinimumRarity:"PROTOTYPE",protectDiscoveries:true,allowSpecialAutomation:false,pauseOnLegendary:false},legendary={rarity:"LEGENDARY",value:500,quality:10};assert.equal(core.decideAutomation(base,legendary,{firstDiscovery:true}).action,null);assert.equal(core.decideAutomation(base,legendary,{firstDiscovery:true}).protected,true);assert.equal(core.decideAutomation(base,legendary).action,"keep");assert.equal(core.decideAutomation({...base,autoKeep:false},{rarity:"COMMON",value:500,quality:2}).action,"sell");assert.equal(core.decideAutomation({...base,autoKeep:false,autoSell:false},{rarity:"COMMON",value:1,quality:2}).action,"scrap");assert.equal(core.decideAutomation({...base,pauseOnLegendary:true},legendary).paused,true);assert.equal(core.decideAutomation({...base,allowSpecialAutomation:true},legendary,{firstDiscovery:true}).action,"keep");
});

test("Chaos Works warehouse comparison respects low stats and sorting",()=>{
  const context={GubuntuGames:{register:()=>{}}};load("games/chaos-works.js",context);const core=context.ChaosWorksCore,a={quality:80,value:200,stats:{strength:80,noise:20,friction:5},createdAt:"2026-01-01"},b={quality:70,value:250,stats:{strength:90,noise:40,friction:10},createdAt:"2026-02-01"},rows=core.compareProducts(a,b);assert.equal(rows.find(row=>row.key==="strength").winner,"b");assert.equal(rows.find(row=>row.key==="noise").winner,"a");assert.equal(rows.find(row=>row.key==="friction").lowerIsBetter,true);assert.strictEqual(core.sortProducts([a,b],"value")[0],b);assert.strictEqual(core.sortProducts([a,b],"quality")[0],a);assert.strictEqual(core.sortProducts([a,b],"newest")[0],b);assert.equal(core.warehouseCapacity({warehouseExpansion:0}),20);assert.equal(core.warehouseCapacity({warehouseExpansion:5}),70);
});
